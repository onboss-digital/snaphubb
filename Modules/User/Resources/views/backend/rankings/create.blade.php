...
@section('content')
    ...
    {{ html()->form('POST', route('backend.users.ranking.store'))
        ...
        html()->checkbox('status', '')
            ...
            ->value(1)
                        ...
                        <div class="col-md-6">
                            ...
                            {{ html()->select('plans[]', $plans, old('plans'))->class('form-control select2')->id('plans')->multiple()->attribute('required','required') }}
                            @error('plans')
                               <span class="text-danger">{{ $message }}</span>
                            @enderror
