...
{{ html()->form('POST', route('backend.videos.store-downloads', $data->id))->attribute('data-toggle', 'validator')->open() }}
@csrf
                ...
                <div class="row gy-3">
                    ...
                    <div id="download_quality_status_section" class="col-md-12 download_quality_status_section {{ old('enable_download_quality', $data->enable_download_quality ?? 0) == 1 ? '' : 'd-none' }}">
                        <div id="video-inputs-container-parent">
                            @if(!empty($data->videoDownloadMappings) && count($data->videoDownloadMappings) > 0)
                                @foreach($data->videoDownloadMappings as $mapping)
