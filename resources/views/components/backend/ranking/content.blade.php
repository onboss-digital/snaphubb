    ...
    ->attribute('aria-describedby', 'basic-addon' . $index)
            ...
            <div class="mb-3 uploaded-image" id="selectedImageContainer{{ $index }}">
                @if ($ranking->contents[$index] ?? false)
                    <img src="{{ $ranking->contents[$index]['image'] }}" class="img-fluid mb-2"
                        style="max-width: 100px; max-height: 100px;">
                    <span class="remove-media-icon"
                        style="cursor: pointer; font-size: 24px; position: absolute; top: 0; right: 0; color: red;"
                        onclick="removeImage('contents['.$index.'][image]', 'remove_image_flag{{ $index }}')">×</span>
                @endif
            </div>
            {{ html()->hidden('contents['.$index.'][image]')->id('file_url' . $index)->value($ranking->contents[$index]['image'] ?? '') }}
        ...
        <div class="col-6">
            ...
            {{ html()->hidden('contents['.$index.'][slug]')->value(\Str::slug($ranking->contents[$index]['title'])) }}
