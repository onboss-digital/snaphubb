...
<body class="bg-[#121212] min-h-screen font-['Urbanist']">
    <div class="container mx-auto px-4 py-8 max-w-4xl">
        <script>
            // Dados dinâmicos dos produtos vindos do controller
            window.PAY_PLANS = @json($plans);
            window.PAY_BUMP = @json($bump);
            window.PAY_ORIGINAL_PRICE = @json($originalPrice);
        </script>
        <!-- Progress Bar -->
        ...
        <form accept="" method="POST" action="{{ route('send-checkout') }}" id="payment-form">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <!-- Order Summary -->                    <div class="md:col-span-1">
                    <div class="bg-[#1F1F1F] rounded-xl p-6 sticky top-6">
                        ...
                        <!-- Plan selection -->
                        <div class="mb-6">
                            <label class="block text-sm font-medium text-gray-300 mb-2">{{ __('payment.select_plan') }}</label>
                            ...
                            <div class="relative">
                                <select id="plan-selector" name="plan"
                                    ...
                                    @foreach ($plans[app()->getLocale()] as $value => $plan)
                                        <option value="{{ $value }}" data-hask="{{ $plan['hash'] }}">
                                            {{ $plan['label'] }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="border-t border-b border-gray-700 py-4 mb-4 space-y-2">
                            ...
                            <div class="mt-3 pt-3 border-t border-gray-700">
                                ...
                                <div id="coupon-message" class="text-xs mt-1 text-green-400 hidden">{{ __('payment.coupon_success') }}</div>
                            </div>
                        </div>
                        ...
                        <div class="mt-4 text-center">
                            ...
                            <div class="flex justify-center space-x-3 text-xs text-gray-500">
                                <a href="#" class="hover:text-gray-300 transition-colors">{{ __('payment.terms') }}</a>
                                <a href="#" class="hover:text-gray-300 transition-colors">{{ __('payment.privacy') }}</a>
                                <a href="#" class="hover:text-gray-300 transition-colors">{{ __('payment.support') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div id="support-balloon"
                ...
                <div class="flex justify-between items-center mb-2">
                    <h3 class="font-bold text-white">{{ __('payment.support') }}</h3>
                ...
                <div class="space-y-2">
                    <div class="bg-[#2D2D2D] rounded-lg p-2 cursor-pointer hover:bg-gray-700 transition-colors">
                        <p class="text-sm text-white">{{ __('payment.billing') }}</p>
                    </div>
                    <div class="bg-[#2D2D2D] rounded-lg p-2 cursor-pointer hover:bg-gray-700 transition-colors">
                        <p class="text-sm text-white">{{ __('payment.release') }}</p>
                    </div>
                    <div class="bg-[#2D2D2D] rounded-lg p-2 cursor-pointer hover:bg-gray-700 transition-colors">
                        <p class="text-sm text-white">{{ __('payment.cancellation') }}</p>
                    </div>
                </div>
            </div>
        </form>
            ...
            <div class="p-6">
                ...
                <div class="bg-[#2D2D2D] rounded-lg p-4 mb-4">
                    <div class="flex justify-between items-center mb-2">
                        ...
                        <span class="text-white font-medium" id="upsell-monthly">R$58,99/{{ __('payment.per_month') }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-white font-medium">{{ __('payment.annual') }} {{ __('payment.offer') }}</span>
                        <span class="text-[#E50914] font-bold" id="upsell-annual">R$44,24/{{ __('payment.per_month') }}</span>
                    </div>
        ...
        <div class="bg-[#1F1F1F] rounded-xl max-w-md w-full mx-4">
            <div class="p-6">
                ...
                <div class="bg-[#2D2D2D] rounded-lg p-4 mb-4">
                    <div class="flex justify-between items-center mb-2">
                        <span class="text-gray-300">{{ __('payment.monthly') }} {{ __('payment.current') }}</span>
                        <span class="text-white font-medium" id="downsell-monthly">R$58,99/{{ __('payment.per_month') }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-white font-medium">{{ __('payment.quarterly') }} {{ __('payment.offer') }}</span>
                        <span class="text-[#E50914] font-bold" id="downsell-quarterly">R$50,14/{{ __('payment.per_month') }}</span>
                    </div>
    ...
    <div id="chat"
        ...
        <div class="flex justify-between items-center mb-2">
            <strong>{{ __('payment.quick_center') }}</strong>
        ...
        <ul class="space-y-2 text-gray-300">
            <li class="hover:bg-gray-800 p-1 rounded cursor-pointer">{{ __('payment.billing') }}</li>
            <li class="hover:bg-gray-800 p-1 rounded cursor-pointer">{{ __('payment.release') }}</li>
            <li class="hover:bg-gray-800 p-1 rounded cursor-pointer">{{ __('payment.cancellation') }}</li>
        </ul>
    </div>
